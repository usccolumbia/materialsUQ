...
### ALL TENSORS MOVED TO CUDA OFF CPU --> WILL CHANGE TO VARIABLE IN THE FUTURE
...
class BNNLayer(nn.Module):
    ...
    def forward(self, X, mode):
        ...
        Z_noise = self._random(_shape).to('cuda')
        #warnings.warn("Warning Z_mean_device: ".join(str(Z_Mean.device)))
        #warnings.warn("Warning Z_Std_device: ".join(str(Z_Std.device)))
        #warnings.warn("Warning Z_noise_device: ".join(str(Z_noise.device)))
        ...
        '''
        warnings.warn("Warning pow_device: ".join(str(torch.pow(X, 2).device)))
        warnings.warn("Warning expand: ".join(str(self.prior_var.expand(self.n_input, self.n_output).to('cuda').device)))
        warnings.warn("Warning total_1_device: ".join(str(torch.mm(torch.pow(X, 2),
                     self.prior_var.expand(self.n_input, self.n_output).to('cuda')).device)))
        warnings.warn("Warning expand_device: ".join(str(self.prior_var.expand(*_shape).device)))
        '''
        Prior_Z_Std = torch.sqrt(
            torch.mm(torch.pow(X, 2),
                     self.prior_var.expand(self.n_input, self.n_output).to('cuda')) +
            self.prior_var.expand(*_shape).to('cuda')
        ).detach()
