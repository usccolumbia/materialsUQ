...
################################################################################
#  Testing error machine learning
################################################################################


def process_data_error(data_path, processed_path, processing_args):
    ...
    ##Load targets
    target_property_file = os.path.join(data_path, processing_args["target_path_errors"])
    print(target_property_file)
    assert os.path.exists(target_property_file), (
        "targets not found in " + target_property_file
    )
    with open(target_property_file) as f:
...
##Fetch dataset using the error targets; processes the raw data if specified
#Always reprocess the data when training because errors are different each time
def get_dataset_error(data_path, target_index, reprocess="False", processing_args=None):
    if processing_args == None:
        processed_path = "processed_errors"
    else:
        processed_path = processing_args.get("processed_path", "processed_errors")
    ...
    process_data_error(data_path, processed_path, processing_args)
    if os.path.exists(os.path.join(data_path, processed_path, "data.pt")) == True:
        dataset = StructureDataset(
            data_path,
            processed_path,
