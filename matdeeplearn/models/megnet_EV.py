...
class LinearNormalGamma(torch.nn.Module):
    def __init__(self, in_chanels, out_channels):
        super().__init__()
        self.linear = torch.nn.Linear(in_chanels, out_channels*4)
    ...
    def forward(self, x):
        ...
        #print(x)
        #print(logv)
        return mu, self.evidence(logv) + min_val, self.evidence(logalpha) + min_val+ 1, self.evidence(logbeta)+ min_val
# Megnet
...
class MEGNet_EV(torch.nn.Module):
    def __init__(
        self,
        data,
        dim1=64,
        ...
        **kwargs
        ...
        if post_fc_count > 0:
            ...
            for i in range(post_fc_count):
                if i == 0:
                    ...
                    self.post_lin_list.append(lin)
            self.lin_out = LinearNormalGamma(dim2, output_dim)
            ...
            if self.pool_order == "early" and self.pool == "set2set":
                ...
                self.lin_out = LinearNormalGamma(post_fc_dim * 3, output_dim)              
            else:
                self.lin_out = LinearNormalGamma(post_fc_dim, output_dim)   
        ...
        if self.pool_order == "early" and self.pool == "set2set":
            ...
            self.lin_out_2 = LinearNormalGamma(output_dim * 2, output_dim)
